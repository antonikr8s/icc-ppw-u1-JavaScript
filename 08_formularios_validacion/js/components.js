...
function MensajeError(mensaje) {
  // 5.2.1: Crear un div con className 'mensaje-error'
  const container = document.createElement('div');
  container.className = 'mensaje-error';

  // 5.2.2: Crear un <strong> con textContent '✗ Error'
  const titulo = document.createElement('strong');
  titulo.textContent = '✗ Error';

  // 5.2.3: Crear un <p> con textContent igual al parámetro mensaje
  const texto = document.createElement('p');
  texto.textContent = mensaje;

  // 5.2.4: Agregar titulo y texto al container con appendChild
  container.appendChild(titulo);
  container.appendChild(texto);

  // 5.2.5: Retornar el container
  return container;
}
...
function ResultadoCard(datos) {
  ...
  // 5.3.1: Iterar cada entrada del objeto datos con Object.entries()
