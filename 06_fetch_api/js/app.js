...
async function cargarPosts() {
    try {
        ...
        listaPosts.appendChild(Spinner()); 
...
async function guardarPost(datosPost) {
  try {
    ...
    if (modoEdicion) {
      ...
      // Agregar el resultado al INICIO del array posts usando unshift()
...
async function eliminarPost(id) {
  // Si retorna false, salir de la función
  ...
  try {
    ...
    // Filtrar el post eliminado del array posts
    posts = posts.filter(p => p.id !== id);
    
    // Filtrar el post eliminado del array postsFiltrados
